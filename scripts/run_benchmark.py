"""
Fast benchmark: mass-asymmetry baseline vs physics-enhanced transformer.

Optimized for CPU execution. Tests with and without pT smearing.
"""
...
def eval_baseline(val_fm, val_labels, num_jets):
    """Evaluate mass-asymmetry baseline on validation set."""
...
def main():
    ...
    for smear in smear_levels:
        ...
        baseline_acc = eval_baseline(val_fm, val_labels, num_jets)
        print(f"  Baseline (min mass asym): {baseline_acc:.4f} ({baseline_acc:.1%})")

        # ML model — use smaller model for speed, larger for 0% to show peak performance
        if smear == 0.0:
            ml_acc, ml_epoch, ml_params, ml_time = train_and_eval(
                ...
                dropout=0.1, lr=3e-4, wd=1e-4, batch_size=256,
                warmup=5, num_epochs=80, patience=20,
    ...
    # Final summary
    n_assign = len(enumerate_assignments(num_jets))
    print(f"\n{'='*60}")
    ...
    print(f"\nKey finding: ML model uses physics-informed features (invariant masses,")
    print(f"mass asymmetry, deltaR) computed directly from four-vectors, combined")
    print(f"with learned transformer embeddings.")
